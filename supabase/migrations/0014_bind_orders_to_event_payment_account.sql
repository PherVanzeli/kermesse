...
declare
  ...
  selected_payment_account_id uuid;
  selected_provider public.payment_provider;
  payment_setting_found boolean := false;
...
begin
  ...
  select
    settings.payment_account_id,
    account.provider,
    true
  into
    selected_payment_account_id,
    selected_provider,
    payment_setting_found
  from public.event_payment_settings settings
  join public.payment_accounts account
    on account.id = settings.payment_account_id
   and account.tenant_id = settings.tenant_id
  where settings.event_id = p_event_id
    and settings.enabled = true
    and account.active = true;

  if not found then
    payment_setting_found := false;
  end if;

  if p_payment_method = 'pix' and not payment_setting_found then
    raise exception using
      errcode = '23514',
      message = 'Este evento ainda não possui um gateway de pagamento Pix configurado.';
  end if;
